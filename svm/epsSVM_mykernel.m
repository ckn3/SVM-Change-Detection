function [model, out_param, kerneloption] = epsSVM_mykernel(instance_matrix,instance_matrixN, instance_matrixM,label_vector, in_param)
...
% in_param.CCC
cmd = ['-t 4 -g 1 -c' ' ' num2str(in_param.other.CCC)];
% cmd
% cmd = '-t 4 -g 1 -c 15';
...
% summation
K = 0.2*K1+0.4*K2+0.4*K3;%indianpines
K = [(1:size(K,1))' K];
