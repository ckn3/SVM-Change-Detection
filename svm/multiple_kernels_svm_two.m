function [ypred_f, out_param] = multiple_kernels_svm_two(varargin)
...
if (scaling_range)
    ...
    Ximg2((nfold-1)*nelem+1:end,:) = (mapminmax('apply',Ximg2((nfold-1)*nelem+1:end,:)',scale_factor2))';%%% for the remaining data set
end
% Scale each feature in order to have std=1
if (scaling_std)
...
[model, out_param,kerneloption] = epsSVM_mykernel_two(double(X)',double(X2)',double(L)',in_param);
...
K1 = makekernel_two(xtest_sspe,double(X)',kerneloption(1),in_param.other.gamma);
K2 = makekernel_two(xtest_sspa2,double(X2)',kerneloption(2),in_param.other.gamma);

K=0.4*K1+0.6*K2;%Èý¸öºË indianpines 0.2 0.1 0.7//0.1 0.2 0.7 labels400   paviaU 0.4 0.3 0.3 labels400
