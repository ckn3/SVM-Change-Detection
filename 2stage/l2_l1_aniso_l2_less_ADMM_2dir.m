...
fixed0 = (fixed==0);
fixed1 = (fixed==1);
N1 = size(F,1); N2 = size(F,2); N3= size(F,3);N = N1*N2;
...
sigma1 = zeros(2*N1,N2); sigma2 = zeros(N1,N2);
l = zeros(2*N1,N2);  w = zeros(N1,N2);
gn = ones(N1,N2); gnp1 = zeros(N1,N2);
g = f;
w(fixed0) = f(fixed0);
iter = 1;
iter_error = norm(gn-gnp1,'fro')/norm(gn,'fro');
while iter_error > 1e-3
