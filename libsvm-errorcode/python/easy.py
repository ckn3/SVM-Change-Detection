...
libsvmpath = "/home/master/93/r93002/libsvm-errorcode/"


is_win32 = (sys.platform == 'win32')
if not is_win32:
	svmscale_exe = libsvmpath+"svm-scale"
	svmtrain_exe = libsvmpath+"svm-train"
	svmpredict_exe = libsvmpath+"svm-predict"
	grid_py = libsvmpath+"python/grid.py"
...
cmd = "%s -svmtrain %s -gnuplot %s -i %s %s" % (grid_py, svmtrain_exe, gnuplot_exe, multiclass_type, scaled_file)
...
cmd = "%s -c %s -g %s -i %s %s %s" % (svmtrain_exe,c,g,multiclass_type,scaled_file,model_file)
print 'Training...'
os.popen(cmd)
